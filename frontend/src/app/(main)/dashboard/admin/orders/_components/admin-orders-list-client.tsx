...
// =============================================================
// FILE: src/app/(main)/dashboard/admin/orders/_components/admin-orders-list-client.tsx
// FINAL — Admin Orders List (DriverPicker ile dinamik sürücü seçimi)
// =============================================================
...
export default function AdminOrdersListClient() {
  ...
  function apply(next: Partial<AdminListOrdersQuery>) {
    ...
    router.push(qs ? `/dashboard/admin/orders?${qs}` : `/dashboard/admin/orders`);
  }
  ...
  function openAssign(o: OrderView) {
    ...
    setDriverId(current);
    setDriverLabel(''); // seçilince dolacak (fallback driverId)
    setAssignNote('');
              ...
              <div key={o.id} className="rounded-md border p-3">
                ...
                <div className="mt-3 grid grid-cols-2 gap-2">
                  <Button asChild variant="outline" size="sm">
                    <Link
                      prefetch={false}
                      href={`/dashboard/admin/orders/${encodeURIComponent(o.id)}`}
                    >
              ...
              <TableBody>
                {rows.map((o) => (
                  <TableRow key={o.id}>
                    ...
                    <TableCell className="text-sm">{formatDateTime(o.created_at as any)}</TableCell>
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-2">
                        ...
                        <Button
                          ...
                          disabled={busy || !canApprove(o)}
                          onClick={() => doApprove(o.id)}
                          title={canApprove(o) ? 'Onayla' : 'Sadece gönderilmiş sipariş onaylanır'}
                        >
                          <CheckCircle2 className="mr-2 size-4" />
                          Onayla
                        </Button>
                        ...
                        <Button
                          ...
                          disabled={busy || !canCancel(o)}
                          onClick={() => openCancel(o)}
                          title={canCancel(o) ? 'İptal et' : 'Teslim/iptal sipariş iptal edilemez'}
                        >
                          <Ban className="mr-2 size-4" />
                          İptal
                        </Button>
